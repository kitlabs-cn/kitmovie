...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 页面管理 <span class="c-gray en">&gt;</span> 页面列表 <a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="pd-20">
	<form action="<?=links('pages','del')?>?ac=all" method="post" class="form form-horizontal" id="myform" name="myform">
	<div class="cl pd-5 bg-1 bk-gray"> <span class="l"> 
	<a class="btn btn-primary radius" href="javascript:;" onclick="cmd('添加页面','<?=links('pages','edit')?>')"><i class="Hui-iconfont">&#xe600;</i> 添加页面</a> 
...
foreach ($array as $row) {
        $zt = ($row->yid==0) ? '<span class="label label-success radius">显示</span>' : '<span class="label label-primary radius">隐藏</span>';
			...
			<tr class="text-c odd">
				...
				<td>'.sub_str($row->text,50).'</td>
				<td>'.$row->bs.'</td>
				<td>'.$zt.'</td>
				<td class="f-14 td-manage">
				    <a style="text-decoration:none" class="ml-5" onClick="cmd(\'编辑修改\',\''.links('pages','edit',0,'id='.$row->id).'\')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a> 
